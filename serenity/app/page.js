...
import { MOOD_LEVELS, getDailyPrompt } from '@/lib/ai'
...
export default function Home() {
  ...
  const loadUserData = (profile) => {
    setUserProfile(profile)
    const convId = storage.getCurrentConversationId()
    if (convId) {
      const conversation = storage.getConversation(convId)
  ...
  const createNewConversation = () => {
    ...
    const initialMessages = [{ role: 'bot', text: "Hello, I'm Serenity. I'm here to listen — no judgment, just presence. How are you feeling today?" }]
    setMsgs(initialMessages)
    storage.saveConversation(newConvId, initialMessages)
  }

  const handleNameSubmit = () => {
    if (!userName.trim()) return
    const profile = storage.createUserProfile(userName)
    setUserProfile(profile)
  ...
  useEffect(() => {
    ...
    const p = Array.from({ length: 40 }, () => ({
      ...
      col: ['rgba(108,92,231,', 'rgba(0,206,201,', 'rgba(162,155,254,'][Math.floor(Math.random() * 3)]
    }))
    let id
    const anim = () => {
      ...
      p.forEach(pt => {
        ...
        if (pt.x < 0 || pt.x > c.width || pt.y < 0 || pt.y > c.height) {
          ...
          pt.y = Math.random() * c.height
        }
        ctx.beginPath()
        ctx.arc(pt.x, pt.y, pt.s, 0, Math.PI * 2)
        ctx.fillStyle = pt.col + pt.o + ')'
        ctx.fill()
      })
  ...
  // Load wellness data and check Ollama
  useEffect(() => {
    fetch('/api/ollama')
      .then(r => r.json())
      .then(d => setOllamaStatus(d.online ? 'ollama' : 'template'))
      .catch(() => setOllamaStatus('template'))
    
    fetch('/api/wellspring')
      .then(r => r.json())
      .then(d => {
        setWellnessData({ breathing: d.breathing || [], grounding: d.grounding || [] })
        setBDetails({ ...d.breathingDetails, ...d.groundingDetails })
        setDailyPrompt(d.dailyPrompt || getDailyPrompt())
      })
      .catch(() => {})
  }, [])
  ...
  const loadMoodHistory = async () => {
    try {
      ...
      if (history.length > 0) {
        const values = history.slice(-14).map(m => m.value || 0)
        const avg = (values.reduce((a, b) => a + b, 0) / values.length).toFixed(1)
        const trend = values[values.length - 1] > (values[0] || 0) ? '📈' : values[values.length - 1] < (values[0] || 0) ? '📉' : '➡️'
        setMoodStats({ average: avg, trend, count: history.length })
      }
    } catch {}
  }
  ...
  const send = useCallback(async () => {
    ...
    const t = input.trim()
    setInput('')
    const newMessages = [...msgs, { role: 'user', text: t }]
    setMsgs(newMessages)
    setLoading(true)
    try {
      const r = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ text: t, userName: userProfile?.name })
      })
      const d = await r.json()
      const updatedMessages = [...newMessages, { role: 'bot', text: d.reply, emotion: d.emotion, source: d.source }]
      setMsgs(updatedMessages)
      if (conversationId) {
        storage.saveConversation(conversationId, updatedMessages)
      }
    } catch {
      const fallbackMsg = { role: 'bot', text: "I'm here to listen. Tell me more about what's on your mind." }
      const updatedMessages = [...newMessages, fallbackMsg]
      setMsgs(updatedMessages)
      if (conversationId) {
        storage.saveConversation(conversationId, updatedMessages)
      }
    ...
    setLoading(false)
  }, [input, loading, msgs, userProfile, conversationId])

  const logMood = async (key) => {
    try {
      const moodData = MOOD_LEVELS.find(m => m.key === key)
      storage.saveMood({ mood: key, value: moodData?.value })
      setMoodMsg('Mood saved!')
      loadMoodHistory()
      setTimeout(() => setMoodMsg(''), 2500)
    } catch {}
  }
  ...
  const saveJournal = async () => {
    ...
    try {
      storage.saveJournalEntry(journalText)
      setJournalText('')
      loadJournalEntries()
    } catch {}
  }
  ...
  if (!mounted || showNamePrompt) {
    return (
      <div className="h-screen flex flex-col items-center justify-center" style={{ background: 'linear-gradient(135deg, #0f0c29, #1a1a4e, #24243e)' }}>
        {mounted && <canvas ref={canvasRef} className="fixed inset-0 pointer-events-none z-0" />}
        <div className="relative z-10 text-center space-y-6 px-6">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-purple-500 to-cyan-400 flex items-center justify-center text-4xl font-bold shadow-lg shadow-purple-500/20 mx-auto">S</div>
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">Welcome to Serenity</h1>
            <p className="text-white/50">Your personal mental wellness companion</p>
          </div>
          <div className="space-y-3 max-w-sm">
            <p className="text-sm text-white/40">What's your name? I'd love to know who I'm talking with.</p>
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-3 rounded-lg bg-white/5 border border-white/10 text-sm text-white outline-none focus:border-purple-500/30 transition-all placeholder:text-white/30"
              />
              <button onClick={handleNameSubmit} disabled={!userName.trim()} className="px-6 py-3 rounded-lg bg-gradient-to-br from-purple-500 to-cyan-400 text-sm font-medium disabled:opacity-30 transition-all hover:scale-105">
                Continue
              </button>
            </div>
          </div>
          <div className="text-[10px] text-white/20">Not a substitute for professional care. <a href="tel:988" className="text-cyan-400/70">988</a></div>
        </div>
      </div>
    )
  }
      ...
      <div className="relative z-10 flex items-center gap-2 p-3 pb-0 overflow-x-auto">
        <div className="flex items-center gap-2 mr-3 flex-shrink-0">
          ...
          <div className="hidden sm:block">
            <div className="text-sm font-bold">Serenity</div>
            <div className="text-[10px] text-white/40">{userProfile?.name && `Hi ${userProfile.name}!`}</div>
          </div>
        </div>
        <NavBtn t="chat" i="💬" />
        <NavBtn t="wellness" i="🧘" />
      ...
      <div className="relative z-10 flex-1 glass m-3 mt-2 flex flex-col overflow-hidden">
        ...
        {tab === 'chat' && <>
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-white/5">
            ...
            <button onClick={createNewConversation} className="text-[10px] px-2 py-1 rounded bg-white/5 hover:bg-white/10 text-white/50 transition-all">New Chat</button>
          </div>
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
            ...
            {loading && (
              <div className="flex anim-in">
                <div className="w-7 h-7 rounded-full bg-gradient-to-br from-purple-500 to-cyan-400 flex items-center justify-center text-xs font-bold mr-2 flex-shrink-0">S</div>
                <div className="px-4 py-3 rounded-2xl chat-bot">
                  <div className="flex gap-1.5">
                    <span className="w-2 h-2 rounded-full bg-white/30 dot" />
                    <span className="w-2 h-2 rounded-full bg-white/30 dot" />
                    <span className="w-2 h-2 rounded-full bg-white/30 dot" />
                  </div>
                </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            ...
            {breathing && (
              <div className="glass p-5 text-center space-y-3">
                ...
                <button onClick={() => { setBreathing(null); setBStep(0); setBCount(0) }} className="px-6 py-2 rounded-full bg-gradient-to-br from-purple-500 to-cyan-400 text-sm font-medium">
                  Stop
                </button>
              </div>
            )}
            ...
            <div>
              <div className="text-xs text-white/30 uppercase tracking-wider mb-2">Breathing Exercises</div>
            ...
            <div>
              <div className="text-xs text-white/30 uppercase tracking-wider mb-2 mt-3">Grounding Techniques</div>
        ...
        {tab === 'mood' && (
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div>
              ...
              <div className="flex items-end gap-0.5 h-16 glass p-3">
                {moodHistory.length > 0 ? (
                  moodHistory.slice(-14).map((m, i) => (
                    <div key={i} className="flex-1 rounded-t relative hover:opacity-80 transition-all" style={{ height: `${((m.value || 0) / 5) * 100}%`, background: MOOD_LEVELS.find(ml => ml.key === m.mood)?.color || '#dfe6e9', minHeight: 4 }}>
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 bg-black/80 text-white px-1.5 py-0.5 rounded text-[9px] whitespace-nowrap opacity-0 hover:opacity-100 transition-opacity pointer-events-none">{MOOD_LEVELS.find(ml => ml.key === m.mood)?.emoji} {m.mood}</div>
                    </div>
                  ))
                ) : (
              ...
              {moodStats.count > 0 && <div className="text-xs text-white/40 mt-2">Average: {moodStats.average}/5 · Trend: {moodStats.trend} · {moodStats.count} entries</div>}
            </div>
          </div>
        )}
        ...
        {tab === 'journal' && (
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {dailyPrompt && (
              <div className="glass p-3 cursor-pointer" onClick={() => setJournalText(dailyPrompt + '\n\n')}>
            ...
            <button onClick={saveJournal} disabled={!journalText.trim()} className="px-5 py-2 rounded-full bg-gradient-to-br from-purple-500 to-cyan-400 text-sm font-medium disabled:opacity-30 transition-all hover:scale-105">
              Save Entry
            </button>
            <div>
              <div className="text-xs text-white/30 uppercase tracking-wider mb-2 mt-1">Recent Entries</div>
              {journalEntries.length > 0 ? (
                journalEntries.slice(0, 5).map((e, i) => (
                  <div key={i} className="glass p-3 mb-2">
                    <div className="text-[10px] text-white/25 mb-0.5">{new Date(e.date).toLocaleDateString()}</div>
                    <div className="text-sm text-white/60 leading-relaxed">{e.text.slice(0, 200)}{e.text.length > 200 ? '...' : ''}</div>
                  </div>
                ))
              ) : (
